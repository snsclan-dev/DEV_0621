...
const $REGEX = {
    ...
    url_image: /^(https?:\/\/|\/\/)[^\s]+?\.(jpg|jpeg|png|gif|bmp|webp|svg)(\?.*)?$/i,
...
const $REGEX_RULES = {
    ...
    tag: { regex: /^(?!,)(?!.*\s)(?!.*,$)(?!.*,,)[ㄱ-ㅎㅏ-ㅣ가-힣\d\w,]{0,50}$/, msg: '태그 : 한글, 영문, 숫자, 공백X, 콤마( , )로 구분 (0~50)' },
    ...
    chat_message: { length: [0, 100], msg: `메세지 : ${$REGEX_MSG}` }, // 수정하자
    // admin    
...
const checkImage = async (folder, input, save)=>{ // input image(thumbnail)
    const $MATCH = input ? input.match($REGEX.url_image) : null;
    const $IMAGE_URL = $MATCH ? $MATCH[0] : null // check https image
    const $IMAGE_TEMP = $REGEX.input_image.test(input) // check temp image
    let $URL;
    try{
        if(input){
            if($IMAGE_URL){
                ...
                const $EXT = $MATCH[2]
                const $FOLDER_IMAGE = path.join('/', process.env.FOLDER, process.env.APP_NAME, 'images', folder, dateFormat('YYMM'))
                const $FOLDER_USER = path.join('/', process.env.FOLDER, process.env.APP_NAME, 'images', folder)
                const $FOLDER = folder === 'profile' ? $FOLDER_USER : $FOLDER_IMAGE;
                const $PATH_IMAGE = path.join($FOLDER, `${dateFormat('YYMMDD_HHmmSSS')}_${$RANDOM}.${$EXT}`)
                fs.access($FOLDER).catch(async ()=>{
                    await fs.mkdir($FOLDER, { recursive: true });
                });
                const $DATA = await axios({ url: input, method: 'GET', responseType: 'arraybuffer' })
                await fs.writeFile($PATH_IMAGE, $DATA.data)
                $URL = $PATH_IMAGE;
            }
            if($IMAGE_TEMP){
