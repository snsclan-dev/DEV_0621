...
export const Chat_Room = ({room})=>{
    ...
    const ROOM_STATUS = useCallback(({ status, notice }) => {
        ...
        // setView((prevView) => {
        //     const $VIEW = [...prevView, { status, notice }];
        //     if ($VIEW.length > 20) $VIEW.splice(0, $VIEW.length - 20);
        //     if ($VIEW.length > 20) $VIEW.slice(-20);
        //     return $VIEW;
        // });
    }, []);
    
    const ROOM_USER = useCallback(({ user }) => { setRoom({ user: user }) }, []);
    ...
    return(<>
        {modal.modify && <Modal title='대화방 설정' setModal={setModal}><Chat_Modify room={$ROOM} user={user} clickStatus={clickStatus} setModal={setModal}/></Modal>}

        <div className="box pd_1 fs_13 mg_b2">
            <p className='line w_80 pd_l'>대기실(현재 위치)은 실시간으로 <span className="c_green fwb">업데이트</span>됩니다.</p>
            <p className='line w_20 ta_r'>{($ADMIN || room.user_id === user.id) && <Link href='/chat/list/1'><button className="bt_3m c_gray">목록</button></Link>}</p>
        </div>

        <div className={`pd_2 mg_b2 ${room.user_id === user.id ? 'box_blue' : 'box_lgray'}`}>
            <div className="box pd_1 mg_b2">
                <p className="pd_l mg_b1 fwb">{$ROOM.title}</p>
                <p className="fs_13"><User_Level level={room.level}/>&nbsp;<span className="c_lblue fwb">{room.name}</span></p>
                
                <div className="pd_h1"><hr /></div>
                <Chat_Room_Type type='info' value={$ROOM.room_type}/>
            </div>
        ...
        {/* {$ADMIN && $ROOM.user.length > 0 && <Kakao_Map user={$ROOM.user}/>} */}
        {$ADMIN && <Kakao_Map user={$ROOM.user}/>}

        <div className="box_lgray pd_2 mg_b2">
            <div className="box pd_1 mg_b2 align">
                <span className="fs_13 c_gray">대화방 참여자 :</span>
