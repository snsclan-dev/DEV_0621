...
import { $USER_MANAGER } from "modules/SYSTEM";
...
export const socketEvent = ({user, setView})=>{
    return {
        ...
        'CHAT_IMAGE': (data) => {
            const { name } = data
            setView((state)=>[...state, data]);
            $CHAT_MESSAGE.line = name === user.name ? $CHAT_MESSAGE.line + 1 : 1;
        },
        'CHAT_VIDEO': (data) => {
            const { name } = data
            setView((state)=>[...state, data]);
...
export const chatSlider = (view)=>{
    ...
    const drag = (clientY)=>{
        ...
        const move = (e)=>{
            ...
            dragY = newY;
            // view.style.height = view.offsetHeight + e.clientY - dragY + 'px';
            // dragY = e.clientY;
        }
        const end = ()=>{
