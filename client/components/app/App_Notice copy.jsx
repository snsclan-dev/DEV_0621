...
export const App_Notice = ({info, user})=>{
    const { app_type, app, app_name, menu, category } = info;
    ...
    const $APP_TYPE_NOTICE = app_type === 'notice'
    const $APP_PUBLIC = { menu: 'app', category: 'notice' } // 공용 파라미터, 타이틀 제거
    
    useEffect(()=>{
        const getNotice = async ()=>{
            const $DATA = await useAxios.get($APP_TYPE_NOTICE ? `/app/notice/${app}/${menu}/${category}/${page}` : `/app/notice/${app}/${$APP_PUBLIC.menu}/${$APP_PUBLIC.category}/${page}`)
    ...
    const clickWrite = async (e)=>{
        ...
        const $CONFIRM = async ()=>{
            const $TYPE = $APP_TYPE_NOTICE ? { app, menu, category } : { app, ...$APP_PUBLIC }
            const $DATA = await useAxios.post(`/app/notice/write`, { ...$TYPE, board_title: input.board_title, editor: $EDITOR.data })
    ...
    return(<div className="max_w100 mg_b2">
        ...
        <div className="box pd_1 mg_b2">
            <p className="line w_70 pd_l fs_13 c_gray">{$APP_TYPE_NOTICE ? info.note : <><span className="c_blue fwb">{app_name}</span><View_Char char='vl'/>공지 및 안내사항</>}</p>
            {$ADMIN && <p className="line w_30 ta_r"><button className="bt_3 c_orange fwb" onClick={()=>{ setWrite(true); setInput({board_title: ''}); }}>⚙️공지 글쓰기</button></p>}
        </div>
        ...
        {list.length ? list.map((e)=> <div key={e.num}>
            ...
            {modal.modify === e.num ? <div className="box_blue pd_1 mg_b2">
                ...
                <div className={`${e.state === $BOARD_STATE['7_view'] ? 'box_red state_bg_red' : 'box_lgray'} pd_1 mg_b2`}>
                    {($APP_TYPE_NOTICE && !$ADMIN) && <h3 className="lh_1 pd_l c_blue fwb">{e.title}</h3>}
                    {$ADMIN && <div className="box_orange pd_1">
                        <h3 className="lh_1 c_orange fwb">⚙️{e.title}</h3>
                    ...
                    {($APP_TYPE_NOTICE || $ADMIN) && <div className="pd_h1"><hr /></div>}
                    <div className={`${$APP_TYPE_NOTICE ? 'min_h10' : 'min_h3'} pd_w`}><Tiptap_Note note={e.note}/></div>
                </div>
            </>}
        </div>) : <div className="box_lgray pd_h3 c_gray ta_c">작성된 공지 및 안내사항이 없습니다.</div>}

        {$APP_TYPE_NOTICE && <Pagination_Click paging={paging} page={setPage}/>}
