...
export const App_Header = ({info, user})=>{
    ...
    const clickGuest = ()=>{ // 손님 로그인
        const $CONFIRM = async ()=>{
            setLoading(1000)
            if($USER.guest){
                setPopup({ msg: '로그아웃 되었습니다.' })
                return setUser({ guest: null })
            }
            const $DATA = await useAxios.get('/main/guest') // 손님 IP 차단 체크
            if($DATA) return setUser({ guest: true })
        }
    ...
    const Login_View = ()=>{
        ...
        return(<p>
            <button className="bt_hidden" onClick={clickGuest}></button>
    ...
    return(<>
        ...
        <div className="header_line">
            <div className="max_w100 mg_0a">
                ...
                <div className="line w_30 ta_r pd_w1 align"><Login_View/></div>
            </div>
        </div>
    </>)
}