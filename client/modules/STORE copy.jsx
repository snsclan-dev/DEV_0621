...
export const storeUser = create((set, get)=>({
    user: { id: null, name: null, location: null }, socket: $SOCKET, accuracy: false,
    initialize: (initialState) => set(initialState),
    getSocket: ()=>{
        const { user } = get()
        $SOCKET.connect()
            ...
            $SOCKET.emit('SOCKET_CREATE', { id: user.id, name: user.name, level: user.level }, (data)=>{
                if(data.code) return storeApp.getState().setPopup({ code: 9, msg: '관리자에게 문의(전달)해 주세요.\ncode: SOCKET_CREATE'})
                    navigator.geolocation.watchPosition((position) => {
                        ...
                        if(!$POSITION.latitude && !$POSITION.longitude) set((state)=> ({ user: { ...state.user, location: { latitude, longitude } } })) // client location
        ...
        $SOCKET.on('disconnect', (reason)=>{
            ...
            storeApp.getState().setPopup({ code: 9, msg: '채팅 서버와 연결이 끊어졌습니다.\n새로 고침이나 다시 접속해 주세요.\ncode: SOCKET_DISCONNECT'})
        })
    },
    setUser: (data) => {
